...
**V1 → V2:** Restricting to groupset-known bikes removes 681 observations with unknown components. The jump from 0.426 to 0.699 adj-R² is partly an artifact of removing a diffuse cluster and partly real (the `groupset_unknown_flag` coefficient of +1.59 in V1 reflects how internally heterogeneous that group is, not a genuine spec effect).
...
The gap is largest in the mid-range and disappears at the top. In V3, SRAM Force carries a coefficient of **+0.98 log pts** above the 105 reference (holding model_series and frame tier constant), while Dura-Ace Di2 is +0.59, indicating the SRAM mid-tier premium is larger than the premium from moving to Shimano's own flagship.

### 2. Model platform is the primary price driver, not specs

Adding `model_series` contributes more to R² than any other single change. After controlling for groupset, frame, and year, platform coefficients relative to Synapse are:
...
The model_series coefficient captures the implicit price of the platform attribute (geometry intent, brand architecture, marketing position) beyond what groupset and frame material explain.
...
In V1 and V2, electronic flag carries +0.21-0.22 log pts (~23% premium). In V3 it collapses to zero (p = 0.87). Di2 and AXS builds cluster within specific model lines (SystemSix Carbon Di2, SuperSix EVO Hi-MOD Dura-Ace Di2) that already carry large positive model_series coefficients. The electronic premium is real, but it's captured by the platform dummy, not the drivetrain flag.
...
## Phase 2: Causal Inference via Difference-in-Differences

### Goal and Design

The hedonic model (V3) answers what explains price differences but cannot answer whether adding electronic drivetrain variants causally raises a model series' average MSRP. I designed a difference-in-differences (DiD) study to isolate that effect from concurrent macro shocks: tariffs, freight costs, the COVID demand surge.

**Design:**
- Treatment: a model series introduces its first electronic-drivetrain variants in a given year
- Control: series with no electronic variants that year
- Method: Two-Way Fixed Effects (TWFE), `log(mean_price_it) ~ treated_post + series_FE + year_FE + macro_controls`

### What the Data Revealed

Running the panel coverage check showed that the Cannondale Wayback backfill begins in **May 2020**, not 2017. The Internet Archive simply hadn't indexed Cannondale's catalogue pages consistently before that date.

That eliminated the pre-treatment window:
...
Without pre-treatment data, the parallel trends assumption cannot be tested and the treatment indicator is nearly collinear with the year fixed effects. The regression ran (n=45 series-year observations) and produced a 95% CI of [-100%, +2M%], which was not a finding. There was no identifying variation in the data.

### Adding Trek

A CDX coverage probe confirmed that Trek's product pages are archived from 2016 onward, with usable snapshot counts each year:
...
The 2,467 figure for 2016 was suspicious. Every other year runs 200-500; 2016 is 10x higher. On inspection, it was a Wayback archive-crawl artifact, not real traffic. I started the Trek backfill from 2017 instead.

Trek's model lines adopted electronic drivetrains at different times between 2017 and 2022. That staggered timing, across models within Trek and between Trek and Cannondale, gave the design treated and control series at different stages of adoption, which is what the DiD requires.

**Cannondale-only TWFE (n=45, preliminary):** -44% point estimate with a 95% CI of [-100%, +2M%], caused by one control series (CAAD Optimo) and treatment collinear with year fixed effects. Not interpretable.
...
**Event study.** Coefficients for each year relative to the year a series first offered electronic groupsets. Pre-period estimates near zero confirm parallel trends held; post-period estimates near zero are the null result.
...
**Plain-language result:** The DiD tests a specific pricing strategy question: do brands use electronic drivetrain launches as an opportunity to reprice an entire model line? The answer is no. The -1.9% estimate (indistinguishable from zero) means introducing Di2 or AXS variants does not shift the series' average MSRP. It adds a higher price point within a tier structure that already existed. Brands price platforms, not drivetrains.

The alternative hypothesis was plausible: a brand could use an electronic launch as cover to raise the floor price of a series, retire low-margin mechanical variants, or reposition the line upward. The data rules that out. The SystemSix Carbon Di2 and SystemSix Carbon 105 coexist at different price points within the same platform; the Di2 launch doesn't pull the 105 price with it. The hedonic regression (V3) shows the same thing from a different angle: `electronic_flag` coefficient collapses to zero (p = 0.87) after adding model_series controls, because drivetrain and price tier are co-determined by platform architecture from the start.

Trek's staggered adoption (Domane SLR 2017, Madone SLR 2019, Domane SL 2019, Checkpoint SL 2022) versus four always-mechanical control series (CAAD Optimo, Checkpoint, Checkpoint ALR, Crockett) is what made the estimate credible. The -44% from the Cannondale-only run was noise from a single control series.

---

## Predictive Validation

To test whether V3 generalizes beyond the sample it was fit on, the groupset-known dataset (n=755) was split into an 80% training set and a 20% holdout, stratified by brand to preserve the Cannondale/Giant ratio.
...
The RMSE of 0.341 log points (~40% in price terms) reflects genuine within-platform variation. A SuperSix EVO Carbon ships in four groupset configurations spanning roughly $2,200 to $7,000, and the model can't resolve which trim a given row represents from spec columns alone, and 0.341 log points is the noise floor of that within-platform spread.
